        ...
        if ($id) {
            ...
            if ($uid) {
                try {
                    ...
                    $key        = 'u@' . $uid;
                    $uinfo      = @json_decode($redis->get($key), true) ?: ['parent' => 0];
                    $log['pid'] = $uinfo['parent'];
                } catch (\Exception $e) {

                }
            }
            log_info($log, 'user.visit');
        }
        ...
        try {
            ...
            if (!$cfg) {
                ...
                if ($rst) {
                    ...
                    $rst = $db->select('cfgid,platform,vercode')->from('{app_version} AS AV')->join('{rest_app} AS RA', 'AV.appkey = RA.appkey')->where([
                        'AV.appkey'      => $this->appKey,
                        'AV.pre_release' => 0
                    ])->desc('AV.vercode')->limit(0, 1)->get();
                    if ($rst) {
                        $cfg    = $table->loadConfig($rst['cfgid'] ? $rst['cfgid'] : 1);
                        $cfg    = $cfg['options'] ? $cfg['options'] : [];
                        $device = $rst['platform'];
            ...
            $sql = $db->select('AV.*,RA.platform')->from('{app_version} AS AV')->join('{rest_app} AS RA', 'AV.appkey = RA.appkey');
            $pkg = $sql->where($uw)->desc('vercode')->limit(0, 1)->get();

            if ($pkg && $pkg['file'] && apply_filter('rest\canUpdate', true, $pkg, $device, $channel, $info)) {
                if ($pkg['platform'] == 'ios') {
                    $url = untrailingslashit(Provider::getShareUrl()) . App::url('rest/ios') . '/' . $this->appKey;
                } else {
                    $url = preg_match('#^(ht|f)tps?://.+$#i', $pkg['file']) ? $pkg['file'] : null;
                }
                //预览发布检测
                $ip      = Request::getIp();
                $restCfg = ConfigurationLoader::loadFromFile('rest');
                $ips     = (array)$restCfg->get('pre_release_ips');
                if ($url && (!$pkg['pre_release'] || in_array($ip, $ips))) {
                    $data['update'] = [
                        'version' => $pkg['version'],
                        'desc'    => $pkg['desc'],
